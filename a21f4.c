...
int main() {
	NodeType StudentsList[NumberOfNodes];
	ListElementType anItem;
	ListPointer FreePtr, List, PredPtr;
	InitializeStoragePool(StudentsList, &FreePtr);
	CreateLList(&List);
	int i = 0;
    int studentsNumber, j;
	printf("Dwse arithmo mathitwn: ");
	scanf("%d" , &studentsNumber);

	while(i<studentsNumber) {
		printf("Dwse arithmo mitrwou gia eisagogi sti lista: ");
		scanf("%d" , &anItem.studentId);
		printf("Dwse vathmo gia eisagogi sti lista: ");
		scanf("%f" , &anItem.grade);
		printf("Dwse ti thesi meta tin opoia tha ginei i eisagogi tou stoixeiou: ");
		scanf("%d", &PredPtr);
		Insert(&List,StudentsList, &FreePtr, PredPtr, anItem);
		TraverseLinked(List, StudentsList);
		i = i +1;
	}
	printf("Dwse ti thesi tou proigoumenou stoixeiou gia diagrafi: ");
	scanf("%d" , &PredPtr);
	Delete(&List, StudentsList, &FreePtr, PredPtr);
	TraverseLinked(List, StudentsList);
...
*/
{
  ...
  if (!EmptyLList(List))
  {
      ...
      printf("plithos stoixeiwn sti lista %d\n", count);
